...
namespace Sriracha.Data.Impersonation.ImpersonationImpl
{
    ...
    public class Impersonator : IImpersonator
    {
        ...
        public ImpersonationContext BeginImpersonation(string userName, string password, string domain=null)
        {
            var maskedCredentials = new DeployCredentialsMasked
            {
                Domain = domain,
                UserName = userName
            };
            ...
            var ok = LogonUser(userName, domain, password, (int)loginType, (int)EnumLogonProvider.LOGON32_PROVIDER_DEFAULT, out handle);
            if (!ok)
            {
                var errorCode = Marshal.GetLastWin32Error();
                string win32message = new Win32Exception(errorCode).Message;
                string errorMessage = string.Format("Could not impersonate {0}, LogonUser returned error code {1}, error message: {2}", maskedCredentials.DisplayValue, errorCode, win32message);
