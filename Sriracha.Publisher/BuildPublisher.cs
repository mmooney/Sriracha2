...
namespace Sriracha.Publisher
{
    public class BuildPublisher
    {
        ...
        private readonly RegexResolver _regexResolver;

        public BuildPublisher(ILog logger)
        {
            _logger = logger;
            _regexResolver = new RegexResolver(logger);
        }
        ...
        public void PublishDirectory(BuildPublishOptions options)
        {
            ...
            _logger.Info(string.Format("Start publishing directory {0} to URL {1}", options.Directory, options.ApiUrl));
            ...
            ZipDirectory(options.Directory, zipPath);
            ...
            {
                _logger.Error(string.Format("Failed to delete ZIP file {0}: {1}", zipPath, err.ToString()), err);
            }
            _logger.Info(string.Format("Done publishing directory {0} to URL {1}", options.Directory, options.ApiUrl));
        }

        private void ZipDirectory(string p, string zipPath)
        {
        ...
        public void PublishFilePattern(BuildPublishOptions options)
        {
            ...
            {
                string x = filePath;
                _logger.Info(string.Format("- {0}", x));
        ...
        public void PublishFile(BuildPublishOptions options)
        {
            ...
            if (!string.IsNullOrEmpty(directoryToDelete))
            {
                ...
                {
                    _logger.Warn(string.Format("Failed to delete directory {0}: {1}", options.File, err.ToString()), err);
                }
            }

            _logger.Info(string.Format("Done publishing file {0} to URL {1}", options.File, options.ApiUrl));
#endif
        }
        ...
        private void PublishZip(string apiUrl, string projectId, string componentId, string branchId, string version, string zipPath, string userName, string password)
        {
            ...
            _logger.Debug(string.Format("Posting file {0} to URL {1}", zipPath, url));
            //var x = client.Send<DeployFileDto>(deployFile);
            var fileToUpload = new FileInfo(zipPath);
            string fileUrl = url + "file";
            //client.Credentials = System.Net.CredentialCache.DefaultCredentials;
            //client.Timeout = 5 * 50 ;
            //client.ReadWriteTimeout = TimeSpan.FromMinutes(5).TotalMilliseconds;
            if (authCookie != null)
            {
                client.CookieContainer.Add(authCookie);
            }
            var fileRequest  = new RestRequest(fileUrl, Method.POST)
                                    .AddFile("fileData", fileToUpload.FullName);
            var fileResponse = client.Execute<ApiDto.ApiBuild>(fileRequest);
            fileResponse.
            ...
            var buildRequest = new ApiDto.ApiBuild
            {
